...
for fname in os.listdir(args.input_dir):
    if fname.startswith(label) and fname.endswith('.csv'):
        filename = os.path.join(args.input_dir, fname)
        break
if not filename:
    raise FileNotFoundError(f"No CSV file found for label {label} in {args.input_dir}")

df = pd.read_csv(filename, sep=';' if label.endswith('-T') else ',')

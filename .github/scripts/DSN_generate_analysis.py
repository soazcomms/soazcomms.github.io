...
# Filter only nighttime (18–06 MST)
mask = (heatmap_df['hour'] >= 18) | (heatmap_df['hour'] < 6)
heatmap_df = heatmap_df[mask].copy()

# Calculate decimal hour and bin every 10 min
heatmap_df['decimal_hour'] = heatmap_df['hour'] + heatmap_df['minute'] / 60
heatmap_df['hour_bin'] = (heatmap_df['decimal_hour'] // (10/60)) * (10/60)
heatmap_df['hour_bin'] = heatmap_df['hour_bin'].round(2)

# Shift hours < 6 to 24–30
heatmap_df['hour_bin_shifted'] = heatmap_df['hour_bin'].apply(lambda h: h if h >= 18 else h + 24)

# Pivot: hour_bin vs date
pivot = heatmap_df.pivot_table(index='hour_bin_shifted', columns='date', values='SQM', aggfunc='mean')

# Configure ticks and labels
pivot = pivot.reindex(sorted(pivot.index))
tick_locs = [i for i, h in enumerate(pivot.index) if abs(h - round(h)) < 0.01]
tick_vals = [pivot.index[i] for i in tick_locs]
tick_labels = [str(int(h) if h < 24 else int(h - 24)) for h in tick_vals]

# Prepare sparse x-axis labels
x_dates = pivot.columns
sparse_xticks = [i for i, date in enumerate(x_dates) if (date.day == 1 or len(x_dates) <= 15 or i % max(len(x_dates)//10, 1) == 0)]
sparse_xticklabels = [str(x_dates[i]) for i in sparse_xticks]

# Plot smaller static seaborn heatmap
fig_heatmap, ax = plt.subplots(figsize=(8, 5))  # reduced size

sns.heatmap(pivot, cmap='viridis', ax=ax, cbar_kws={'label': 'SQM'},
            xticklabels=False, yticklabels=False)

# Titles and axes
ax.set_title("Heatmap of SQM (10-min bins, MST 18:00–06:00)", fontsize=12)
ax.set_xlabel("Date", fontsize=10)
ax.set_ylabel("Local Hour (MST)", fontsize=10)
ax.set_yticks(tick_locs)
ax.set_yticklabels(tick_labels, rotation=0, fontsize=7)

# Set sparse x-ticks manually
ax.set_xticks(sparse_xticks)
ax.set_xticklabels(sparse_xticklabels, rotation=45, fontsize=7)

plt.tight_layout()
fig_heatmap.savefig("plot4_heatmap.png", dpi=150)
plt.close(fig_heatmap)

# HTML block for iframe
with open("plot4_heatmap.html", "w") as f:
    f.write("""
    <h2>4. Heatmap (SQM from 18:00 to 06:00 MST, 10-min bins)</h2>
    <img src="plot4_heatmap.png" width="100%" height="400" alt="Heatmap of SQM values">
    """)

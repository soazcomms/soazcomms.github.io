...
# Floor timestamp to 10-min intervals
heatmap_df['time_bin'] = heatmap_df['MST'].dt.floor('10min')

# Create pivot table
pivot = heatmap_df.pivot_table(index='hour_shifted', columns='time_bin', values='SQM', aggfunc='mean')
...
fig_heatmap = px.imshow(
    ...
    x=pivot.columns,
    y=pivot.index,
    labels=dict(x="Date", y="Hour (MST)", color="SQM"),
    title="4. Interactive Heatmap: SQM Readings (18h–6h MST)",
...
fig_heatmap.update_yaxes(
    ...
    ticktext=[f"{h%24:02d}" for h in range(18, 30)],
    title="Hour (MST)"
)

fig_heatmap.update_xaxes(
    tickformat="%Y-%m",         # Show month
...
# ─── Also Save as PNG ────────────────────────────────────────
fig_heatmap.write_image("plot4_heatmap.png", width=1000, height=500)
#
# Sigma histogram

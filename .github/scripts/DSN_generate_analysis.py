...
# Y-axis: convert minutes back to MST hour
y_ticks = ax.get_yticks()
y_labels = [(18 + int(m / 60)) % 24 for m in y_ticks]
ax.set_yticklabels([f"{h:02d}:00" for h in y_labels])

# X-axis: readable date labels
x_labels = [str(d) for d in pivot.columns]
ax.set_xticklabels(x_labels, rotation=45, ha='left', fontsize=8)

ax.set_title("SQM Heatmap (18:00–06:00 MST)", fontsize=13)
ax.set_xlabel("Date")

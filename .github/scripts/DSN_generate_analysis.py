...
# Pivot table of mean SQM: rows = time bins, cols = dates
...
# Sparse x-ticks on 1st of month or fallback
...
# Format axes
ax.set_title("Heatmap of SQM (10-min bins, MST 18:00–06:00)", fontsize=13)
ax.set_xlabel("Date", fontsize=11)
ax.set_ylabel("Hour (MST)", fontsize=11)
ax.set_yticks([pivot.index.get_loc(h) for h in yticks if h in pivot.index])
ax.set_yticklabels([l for i, l in enumerate(yticklabels) if yticks[i] in pivot.index],
...
# Write HTML file to embed PNG
with open("plot4_heatmap.html", "w") as f:
    f.write("""
<h2>4. Heatmap (SQM from 18:00 to 06:00 MST, 10-min bins)</h2>
<img src="plot4_heatmap.png" width="100%" height="600" alt="Heatmap of SQM values">
""")

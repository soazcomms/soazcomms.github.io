...
fig_jelly = px.density_heatmap(
    ...
    labels={"hour_shifted": "Nightly MST", "NSB": "Radiance (mag/arcsec²)"},
)
...
ax.set_title("Heatmap of SQM Readings", fontsize=14)
ax.set_xlabel("MST")
...
# --- HTML: Plotly from same pivot ---
fig_heatmap_html = px.imshow(
    pivot.values,
    x=pivot.columns,
    y=pivot.index,
    labels={"x": "MST", "y": "NSB mag/arcsec²"},
    title="Heatmap of SQM Readings"
)
fig_heatmap_html.update_layout(margin=dict(l=40, r=20, t=40, b=40))
...
pio.write_html(fig_jelly, file="plot3_jellyfish.html",
               include_plotlyjs='cdn', full_html=False)
pio.write_html(fig_heatmap_html, file="plot4_heatmap.html",
               include_plotlyjs='cdn', full_html=False)

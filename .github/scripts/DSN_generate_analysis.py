...
H_flat = H.flatten()
cdf = np.cumsum(H_flat)
cdf_normalized = (cdf - cdf.min()) * 255 / (cdf.max() - cdf.min())
H_eq_flat = np.interp(H_flat, np.unique(H_flat), np.interp(np.unique(H_flat), cdf, cdf_normalized))
H_eq = H_eq_flat.reshape(H.shape)
...
fig_jelly = go.Figure(data=go.Heatmap(
    ...
    colorscale='Hot',
    colorbar=dict(title='Equalized Count')
))

# Format x-axis ticks from 18 to 30 mapped to 18–6
tick_vals = list(range(18, 31))
tick_text = [str(h if h < 24 else h - 24) for h in tick_vals]

fig_jelly.update_layout(
    title="Jellyfish NSB in mag/arcsec² (Histogram Equalized)",
    xaxis=dict(
        tickmode="array",
        tickvals=tick_vals,
        ticktext=tick_text,
    ...
    yaxis=dict(title="NSB mag/arcsec²", autorange="reversed")
)
...
sns.heatmap(pivot, cmap='viridis', ax=ax, cbar_kws={'label': 'mag/arcsec²'},

...
# Convert UTC to MST and extract hour
heatmap_df = df_all.copy()
heatmap_df['MST'] = heatmap_df['UTC'].dt.tz_convert('America/Phoenix')
# Floor to the hour for accurate binning
heatmap_df['hour_timestamp'] = heatmap_df['MST'].dt.floor('h')
heatmap_df['hour_of_day'] = heatmap_df['MST'].dt.hour

# Pivot table: average SQM per hour of day, per timestamp
pivot = heatmap_df.pivot_table(
    index='hour_of_day', columns='hour_timestamp', values='SQM', aggfunc='mean'
)

# --- PNG output using seaborn ---
fig, ax = plt.subplots(figsize=(14, 6))
sns.heatmap(pivot, cmap='magma', ax=ax)
ax.set_title("Heatmap: SQM by Hour and Timestamp (Local MST)")
...
fig_html = px.imshow(
    pivot.values,
    x=[col.strftime("%Y-%m-%d %H:%M") for col in pivot.columns],
    y=pivot.index,
    labels={"x": "Timestamp (MST)", "y": "Hour (MST)", "color": "SQM"},
    title="Heatmap of SQM by Hour and Time (MST)"
)

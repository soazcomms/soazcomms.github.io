...
def format_hour_axis(x, _):
    h = int(x) // 60
    return f"{h:02d}"

ax.yaxis.set_major_formatter(ticker.FuncFormatter(format_hour_axis))
...
# Compute minute-of-night from 18:00 MST to 06:00 MST
heatmap_df['hour'] = heatmap_df['MST'].dt.hour
heatmap_df['minute'] = heatmap_df['MST'].dt.minute
heatmap_df['minute_of_night'] = heatmap_df['hour'] * 60 + heatmap_df['minute']
heatmap_df['minute_of_night'] = \
    heatmap_df['minute_of_night'].apply(lambda x: x - 18*60
                                        if x >= 18*60 else x + 6*60)

# Only keep data from 18:00 to 06:00
heatmap_df = heatmap_df[(heatmap_df['minute_of_night'] >= 0) &
                        (heatmap_df['minute_of_night'] <= 720)]

# Format time for x-axis
heatmap_df['time_label'] = heatmap_df['MST'].dt.strftime('%Y-%m-%d %H:%M')

# Pivot for seaborn heatmap
pivot = heatmap_df.pivot_table(index='minute_of_night',
                               columns='time_label', values='SQM',
                               aggfunc='mean')
# Plot
fig_heatmap, ax = plt.subplots(figsize=(14, 6))
sns.heatmap(pivot, ax=ax, cmap="viridis", cbar_kws={'label': 'SQM'},
            xticklabels=30, yticklabels=60)

# Axis labels
ax.set_title("NSB Heatmap)", fontsize=14)
ax.set_xlabel("Date and Time (MST)")
ax.set_ylabel("Hour (MST)")

# Format Y-axis to show HH only
y_ticks = ax.get_yticks()
y_labels = [f"{(18 + (int(m) // 60)) % 24:02d}" for m in y_ticks]
...
# HTML output (embed PNG image)
with open("plot4_heatmap.html", "w") as f:
    f.write(f"""
    <h2>4. Heatmap (SQM, 18h–6h MST)</h2>
    <img src="plot4_heatmap.png" alt="NSB Heatmap" width="100%">
    """)

...
# Filter only 18:00–06:00 range
heatmap_df = heatmap_df[(heatmap_df['minute_of_night'] >= 0) & (heatmap_df['minute_of_night'] < 720)]

# Pivot table: index = 10-min bins (minute_of_night), columns = date, values = mean SQM
heatmap_df['minute_bin'] = (heatmap_df['minute_of_night'] // 10) * 10  # 10-min bins
pivot = heatmap_df.pivot_table(index='minute_bin', columns='date', values='SQM', aggfunc='mean')
...
# Y-ticks: one per hour (60 minutes)
yticks = np.arange(0, 721, 60)
yticklabels = [f"{(18 + i)%24:02d}" for i in range(len(yticks))]
ax.set_yticks(yticks)
ax.set_yticklabels(yticklabels, fontsize=9)

# X-ticks
ax.set_xticklabels([str(date) for date in pivot.columns], rotation=45, ha='right', fontsize=8)

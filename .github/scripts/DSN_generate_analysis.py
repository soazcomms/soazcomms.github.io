...
heatmap_df['hour_bin'] = (heatmap_df['hour_shifted'] // (1/6)) * (1/6)  # 10-minute bins

# Create pivot: Y = hour_bin, X = time_bin
...
# Y-axis: format as HH, reverse so 18-23 then 0-6
ax.set_yticks(pivot.index)
ax.set_yticklabels([f"{int(h)%24:02d}" for h in pivot.index])
ax.invert_yaxis()
ax.set_ylabel("Hour (MST)")
plt.title("Heatmap of Night Sky Brightness (10-min avg, MST)")
...
# Save static HTML wrapper for viewing
...
<body>
    <img src="plot4_heatmap.png" alt="Heatmap" style="width:100%; max-height:1000px;">
</body>
</html>""")
#
# Sigma histogram

...
heatmap_df = df_all.copy()
heatmap_df['MST'] = heatmap_df['UTC'].dt.tz_convert('America/Phoenix')
heatmap_df['time_bin'] = heatmap_df['MST'].dt.floor('10min')  # ⬅️ 10-min bins
heatmap_df['date'] = heatmap_df['MST'].dt.date

# Minute-of-night calculation
...
heatmap_df['minute_of_night'] = heatmap_df['minute_of_night'].apply(
    lambda x: x - 18 * 60 if x >= 18 * 60 else x + 6 * 60
)

# Keep only 18:00 to 06:00 range
heatmap_df = heatmap_df[
    (heatmap_df['minute_of_night'] >= 0) & (heatmap_df['minute_of_night'] <= 720)
]

# Pivot table: 10-min bins
pivot = heatmap_df.pivot_table(
    index='minute_of_night',
    columns='date',
    values='SQM',
    aggfunc='mean'
)
...
sns.heatmap(
    ...
    cbar_kws={'label': 'SQM'},
    xticklabels=1,
    yticklabels=12  # 12 ticks across 720 mins = every ~60 mins
)

# Y-axis ticks every hour, but finer binning
tick_positions = np.arange(0, 721, 60)  # Every hour in minutes
tick_labels = [f"{(18 + h) % 24:02d}:00" for h in range(len(tick_positions))]
ax.set_yticks(tick_positions)
ax.set_yticklabels(tick_labels, fontsize=8)
ax.invert_yaxis()

# X-axis labels
ax.set_xticklabels([str(d) for d in pivot.columns], rotation=45, ha='right', fontsize=8)

# Titles
ax.set_title("SQM Heatmap (10-min bins, 18:00–06:00 MST)", fontsize=14)
ax.set_xlabel("Date")

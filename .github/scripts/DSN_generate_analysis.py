...
heatmap_df = df_all.copy()
heatmap_df['MST'] = heatmap_df['UTC'].dt.tz_convert('America/Phoenix')
# Create 15-minute bins from MST datetime
heatmap_df['time_bin'] = heatmap_df['MST'].dt.floor('15min')

# Filter only data between 18:00–06:00 MST
...
# Extract just date for X-axis
heatmap_df['date'] = heatmap_df['MST'].dt.date

# Pivot: rows = minute of night (0–720), columns = date, values = mean SQM
pivot = heatmap_df.pivot_table(
...
# Format axes
ax.set_title("SQM Heatmap (MST 18h–6h)", fontsize=14)
ax.set_xlabel("Date")
ax.set_ylabel("MST Hour")
...
x_dates = [str(d) for d in pivot.columns]
ax.set_xticklabels(x_dates, rotation=45, ha='right', fontsize=8)
...
with open("plot4_heatmap.html", "w") as f:
    f.write("""
    <h2>4. Heatmap (SQM from 18h to 6h MST)</h2>

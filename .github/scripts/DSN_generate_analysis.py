...
run_hours = len(df_local) / 4  # assuming 15-min bins
night_df = df_local[(df_local['Local'].dt.hour >= 18) | (df_local['Local'].dt.hour <= 6)]
night_hours = len(night_df) / 4
...
fig_jelly = px.density_heatmap(
    ...
    title="Jellyfish Plot (Time vs SQM)", labels={"hour_shifted": "Hour (18–06 MST next day)"}
)
...
# Sigma histogram
fig_sigma = px.histogram(df_all, x='chisquared', nbins=100, title="Sigma (chisquared) Histogram")

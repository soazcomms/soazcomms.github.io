...
def rename_files_and_update_table(sqm_folder, dsn_table_path):
    """Rename files using the Site name and update DSNtable.csv with incremented sequence numbers."""
    if not os.path.exists(dsn_table_path):
    ...
    row = dsn_table.loc[dsn_table['Site'] == site_name]

    if row.empty:
        print(f"Site {site_name} not found in DSNtable.csv, skipping.")
        return
    ...
    for filename in os.listdir(sqm_folder):
        ...
        seq_number += 1
        dsn_table.loc[dsn_table['Site'] == site_name, 'Sequence'] = seq_number  # Update sequence
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python DSNrename_sqm_files.py <SQM_FOLDER> <DSNTABLE_CSV>")
        sys.exit(1)

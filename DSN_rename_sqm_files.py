...
def extract_year_from_file(filepath):
    """Extracts the last two digits of the year (YY) from the first data line if .dat or .csv;
    otherwise from the filename (e.g., *_25*)."""
            ...
            with open(filepath, 'r') as file:
                for line in file:
                    if not line.startswith('#'):
                        match = re.search(r'20(\d{2})\d{4}', line)  # Matches 2025xxxx
                        if match:
                            return match.group(1)
        except Exception as e:
            print(f"Error reading file {filepath}: {e}")
    else:
        match = re.search(r'_?(\d{2})[^0-9]', os.path.basename(filepath) + '_')  # ensure trailing non-digit
        if match:
            return match.group(1)

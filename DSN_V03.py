...
#idash=site_file.rfind('-')
matches = process.extract(site_file[:iunder], site_dict, limit=1)
site_number=matches[0][2]+1
DSN_name=site_file[:iunder]
iunder1=DSN_name.rfind('_')+1
site_name=DSN_name[iunder1:]
inf_file=site_file[:idot]
...
if sensor_name == 'SQM1': # .xlsx data
    ...
    if sensor_name == 'TESS':
        ...
        if len_cols>12:  # G96 format
            frame_cols=['UT','Tloc','SQM','Etempc','Stempc']
            drop_cols=list(range(2,5))+list(range(6,15))+[16] \
                +list(range(18,33))
...
if 'TESTING' in os.environ:
    if os.environ['TESTING']: 
        ...
        print("Real influx file: ",INFpath + site_name + ".csv")
else:

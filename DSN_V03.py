...
if "TESTING" in os.environ and os.environ["TESTING"]:
    DSNsites_path = os.getenv("DSNdata")   # your old working test behavior
...
# Check input file for comment lines, determine separator,
# and determine read interval
n_comments = 0
FMT = ""
...
if not in_file.endswith('.xlsx'):
    ...
    # ------------------------------------------------------------
    # Determine separator ONCE from first non-blank, non-comment line
    # ------------------------------------------------------------
    with open(in_file, 'r') as f:
        for line in f:
            ...
            if ',' in line:
                ...
                # whitespace-delimited file (.table, etc.)
                sepcol = None
    ...
    with open(in_file, 'r') as f:
        for line in f:
            ...
            if sepcol is None:
                first = line.split(None, 1)[0].strip().strip('"')
            else:
                first = line.split(sepcol, 1)[0].strip().strip('"')
            # Skip header/non-data lines
            if not first[:4].isdigit():
                n_comments += 1
                continue

            # We now have a real data record
            ndata_read += 1

            # Ignore first 10 valid data records
            if ndata_read <= 10:
                continue

            # Determine timestamp format from first usable record
            if FMT == "":
                if "T" in first:
                    FMT = "%Y-%m-%dT%H:%M:%S.%f"
                else:
                    FMT = "%Y-%m-%d %H:%M:%S"
            try:
                times.append(dt.strptime(first, FMT))
            except ValueError:
                print("Bad timestamp:", repr(first))
                continue

            # Need 51 timestamps to calculate 50 intervals
            if len(times) == 51:
                break

    if len(times) < 51:
        print("Only found", len(times),
              "usable timestamps after first 10 entries; need 51, QUIT.")
        sys.exit(0)

    # ------------------------------------------------------------
    # Calculate mode of first 50 consecutive time intervals
    # ------------------------------------------------------------
    deltas = np.array([
    ...
    mode_delta = values[np.argmax(counts)]

    read_delta = mode_delta / 60.

    print("Found ", n_comments, " comment/header lines")
    print("Read delta mode:", mode_delta, "sec =", read_delta, "min")
...
nearest_delta = min((1., 5., 10.), key=lambda x: abs(read_delta - x))
...
#    print(frame_sensor.head(50))
    ...
    if sepcol is None:
        frame_sensor = pd.read_csv(in_file,header=None,skiprows=ihead,
            sep=r'\s+'
        )
    else:
        frame_sensor = pd.read_csv(in_file,header=None,skiprows=ihead,
    ...
    if sensor_name == 'SQM3': # HG astropy .table format
        # columns wanted from .table:
        # 0 = UTC time
        # 1 = local time
        # 7 = SB  (mag/arcsec^2)
        # 8 = T   (sensor temperature)
        frame_sensor = frame_sensor.iloc[:, [0, 1, 7, 8]].copy()
        frame_cols = ['UT', 'Tloc', 'SQM', 'Etempc']
    elif sensor_name == 'SQM4':
        frame_cols=['UT','Tloc','Etempc','number','freq','SQM'] # Winer
    # relabel columns
    frame_sensor.columns=frame_cols

if nearest_delta < 1: # for MtLemmon G96
    nearest_delta=1
    
if nearest_delta == 1: # for normal TESS, JB special, MtLemmon G96, sample every 5 min
    df=frame_sensor
    frame_sensor = df[df.index % 5 == 0] 
    frame_sensor.reset_index(inplace=True,drop=True)
    ndata=19
print("Read interval: ",nearest_delta," min. Points for chisquared: ",ndata)
#
# for TESS1, period is 1 min, bring it down to 5 min

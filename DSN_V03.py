    ...
    with open(in_file, 'r') as f:
        for line in f:
            ...
            if not line:
                continue
            print("MIERDA")
            # Determine separator from first non-comment line.
            # This must be done BEFORE splitting off the timestamp.
            if ndata_read == 0 and len(times) == 0:
                if ',' in line:
                    sepcol = ','
                elif ';' in line:
                    sepcol = ';'
                else:
                    print("Cannot determine separator, QUIT.")
                    sys.exit(0)

            first = line.split(sepcol, 1)[0].strip()
            print(" FUCKING SEP ",sepcol)
            # skip headers / non-data lines
            if first == 'UTC time' or not first[:4].isdigit():
                n_comments += 1
                continue
            ...
            # 51 timestamps produce 50 pairs
            if len(times) == 51:
                break

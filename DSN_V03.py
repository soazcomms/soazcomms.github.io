...
if sensor_name == 'SQM1': # .xlsx data
    ...
    if sensor_name == 'SQM3': # G96,V06 format
        ...
        frame_cols=['Tloc','SQM','Stempc']
        frame_sensor.columns=frame_cols
        lenS = len(frame_sensor)
        local_time = [frame_sensor.Tloc.iloc[i] for i in range(lenS)]
        local_tz = pytz.timezone("America/Phoenix")
        # Localize the datetime object to your local timezone
        local_time_dt = [dt.strptime(local_time[i],
                            "%Y-%m-%dT%H:%M:%S.%f") for i in range(lenS)]
        # Convert the localized datetime object to UTC
        localized = [local_tz.localize(dt_obj) for dt_obj in local_time_dt]
        utc_time = [localized[i].astimezone(pytz.utc) for i in range(lenS)]
        frame_sensor.insert(0, "UT", utc_time)
        frame_cols=['UT','Tloc','SQM','Etempc']
...
df.moonalt=moonalt
df.sunalt=sunalt
#df.UTC = [dt.strptime(str(df.UTC.iloc[i]), "%Y-%m-%dT%H:%M:%S")
#          for i in range(len(df))]
print("df.UTC[0] :",df.UTC[0])
df.UTC = [dt.strptime(str(df.UTC.iloc[i]).split('+')[0], "%Y-%m-%dT%H:%M:%S.%f") for i in range(len(df))]
